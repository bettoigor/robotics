#!/usr/bin/env python
# -*- coding: utf-8 -*-

#############################################
#                                           #
# Joypad control node for SoyBot            #
#                                           #
# Author: Adalberto Oliveira                #
# Mastering in robotic - PUC-Rio            #   
# Version: 1.0                              #
# Date: 2-13-2019                           #
#                                           #
#############################################

import rospy, time, angles, math, tf2_geometry_msgs, tf, sys, cv2
import tf2_ros as tf2
import numpy as np
from operator import itemgetter, attrgetter, methodcaller
from geometry_msgs.msg import Twist
from sensor_msgs.msg import Joy,Image, CameraInfo
from std_msgs.msg import Bool, Float64
from cv_bridge import CvBridge


def callback_joy(msg):

    global joyCommand
    global vel
    global steer
    global start
    global change_state
    global auto
    global cmd_vel 


    joyCommand = msg
    vel -= msg.axes[4]
    if vel >= 25: vel = 25
    if vel <= -25: vel = -25

    vel += (10* msg.axes[5])
    cmdVel = Twist()

    if msg.buttons[0]:
        vel = 0
        steer = 0
    
    if msg.buttons[9]:
        start = True
        change_state(State.IDLE)
        change_state(State.MANUAL)

    steer = round((msg.axes[2] * 0.25),4)


    if msg.buttons[1]:
        vel = 0
        steer = 0
        start = False 
        change_state(State.IDLE)   
        cmdVel.linear.x = 0  
        cmdVel.angular.z = 0
        cmd_vel.publish(cmdVel)   


    if msg.buttons[5]:
        change_state(State.IDLE)
        vel = 0
        steer = 0
        cmdVel.linear.x = 0  
        cmdVel.angular.z = 0
        cmd_vel.publish(cmdVel)

    if msg.buttons[8]:
        change_state(State.AUTONOMOUS)    
        auto = True
        vel = 0
        steer = 0
        cmdVel.linear.x = 0  
        cmdVel.angular.z = 0
        cmd_vel.publish(cmdVel)   

    if msg.buttons[3]:
        save_image()



#Callback da camera top
def image_callback_top(data):
        global imagem_top, bridge
        imagem_top = bridge.imgmsg_to_cv2(data, desired_encoding = "passthrough")

#Callback da camera bottom0
def image_callback_bottom0(data):
        global imagem_bottom0, bridge
        imagem_bottom0 = bridge.imgmsg_to_cv2(data, desired_encoding = "passthrough")

#Callback da camera bottom1
def image_callbackb_bottom1(data):
        global imagem_bottom1, bridge
        imagem_bottom1 = bridge.imgmsg_to_cv2(data, desired_encoding = "passthrough")

#Callback da camera side0
def image_callback_side0(data):
        global imagem_side0, bridge
        imagem_side0 = bridge.imgmsg_to_cv2(data, desired_encoding = "passthrough")

#Callback da camera side1
def image_callback_side1(data):
        global imagem_side1, bridge
        imagem_side1 = bridge.imgmsg_to_cv2(data, desired_encoding = "passthrough")    

def state_callback(msg):
    global state

    state = msg.activeState

def save_image():
    global img_path

    time = rospy.Time.now()

    cv2.imwrite(img_path+'top/image_cam_{}_{}.png'.format('top', time), imagem_top)
    cv2.imwrite(img_path+'bottom_right/image_cam_{}_{}.png'.format('bottom_right',time), imagem_bottom0)
    cv2.imwrite(img_path+'bottom_left/image_cam_{}_{}.png'.format('bottom_left',time), imagem_bottom1)
    cv2.imwrite(img_path+'side_right/image_cam_{}_{}.png'.format('side_right',time), imagem_side0)
    cv2.imwrite(img_path+'side_left/image_cam_{}_{}.png'.format('side_left',time), imagem_side1)


def send_cmd():
    global vel
    global steer
    global start
    global cmd_vel 
    global state

    cmdVel = Twist()

    cmdVel.linear.x = vel*0.001
    if cmdVel.linear.x > 0.25: cmdVel.linear.x = 0.25  
    if cmdVel.linear.x < -0.25: cmdVel.linear.x = -0.25  
    cmdVel.angular.z = steer

    if not start or state != State.MANUAL:
        vel = 0
        steer = 0
        cmdVel.linear.x = 0  
        cmdVel.angular.z = 0
        print "command do not sent\n"

    else:
        print "send command...\n"
        cmd_vel.publish(cmdVel)
    
    print cmdVel



def robot_joy():

    global cmd_vel 
    global tfBuffer  
    global cv_image
    global change_state

    rospy.init_node('soybot_joy', anonymous=True) #nome do nó
    
    # Publishers
    cmd_vel = rospy.Publisher('cmd_vel', Twist, queue_size=10)
    pub_heartbeat = rospy.Publisher('heartbeat', Heartbeat, queue_size=1)
    
    # Subscribers
    rospy.Subscriber('joy', Joy, callback_joy)  
    '''
    rospy.Subscriber('image_raw_0', Image, image_callback_top)  
    rospy.Subscriber("image_raw_1", Image, image_callback_bottom0)   #Criado agora
    rospy.Subscriber("image_raw_2", Image, image_callbackb_bottom1)   #Criado agora
    rospy.Subscriber("image_raw_3", Image, image_callback_side0)       #Criado agora
    rospy.Subscriber("image_raw_4", Image, image_callback_side1)       #Criado agora
    rospy.Subscriber("active_state", State, state_callback)       #Criado agora

    # Services
    rospy.wait_for_service('change_state')
    change_state = rospy.ServiceProxy('change_state',ChangeState)
    '''

    rate = rospy.Rate(30) 
    time.sleep(2)

    

    while not rospy.is_shutdown():
        send_cmd()
        rate.sleep()


# Main
# GLOBAL VARIABLES
img_path = 'local' #sys.argv[1]
joyCommand = Joy()
bridge = CvBridge()
imagem_top = np.array([[0],[0],[0]])
imagem_bottom0 = np.array([[0],[0],[0]])
imagem_bottom1 = np.array([[0],[0],[0]])
imagem_side0 = np.array([[0],[0],[0]])
imagem_side1 = np.array([[0],[0],[0]])
vel = 0
steer = 0
start = False
auto = False
state = 0

if __name__ == '__main__':
    try:
        robot_joy()
    except:
        cv2.destroyAllWindows()         
        print 'Node ended.'

# End Main